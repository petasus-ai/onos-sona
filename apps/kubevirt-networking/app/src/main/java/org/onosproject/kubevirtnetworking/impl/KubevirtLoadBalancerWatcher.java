...
public class KubevirtLoadBalancerWatcher extends AbstractWatcher {
    ...
    private synchronized void instantiateWatcher() {
        ...
        try {
            watch = client.genericKubernetesResources(lbCrdCxt).inAnyNamespace().watch(watcher);
        } catch (Exception e) {
    ...
    private class InternalLoadBalancerWatcher implements Watcher<GenericKubernetesResource> {
